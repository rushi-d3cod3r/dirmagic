...
confirm_delete() {
    echo -e "$bold $red Warning: This action will delete files. Please review before confirming."
        ...
        y|Y|yes|YES)
            ...
            while IFS= read -r file; do
                ...
                if [[ " ${excluded_extensions[@]} " =~ " ${extension} " ]]; then
                    ...
                    # Delete the file if it's not in the exclude list
                    rm -f "$file"
            ...
            echo -e "$bold $red Original files and empty folders permanently deleted."
            ;;
        *)
